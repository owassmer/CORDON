...
A further implementation round follows only when a repair changed meaning or a material risk remains open, and rounds continue until the judgment can be written. A later round is answered by the reviewer that raised the findings, carrying its previous round, under the round brief and with the adjudication of that round. That reviewer knows what its findings meant, so it can tell a faithful cure from a cosmetic one; it still reads the whole change again. Fresh reviewers given nothing sample new objections and never judge whether the earlier ones were cured. Ordinarily the reviewer's session is resumed; where it cannot be, a fresh reviewer receives that reviewer's previous report with the adjudication. A fresh reviewer given only the adjudication replaces a resumed one when the unit is re-planned, or when the adjudication finds that reviewer's own framing is what prevents judgment; that adjudication names the findings it thereby retires. A second plan review follows the same rule. A falling residual is not that judgment, and neither is a round count. A unit the adjudicator still cannot judge after repeated rounds is re-planned rather than reviewed again; whether recurrence across rounds indicts the structure is decided under `DESIGN_PRINCIPLES.md`, "Minimum sufficient structure", which asks first whether the earlier rule was sound and merely unapplied.

Owen merges the pull request. Merge accepts the unit's content as reviewed; changed content needs its own acceptance.

## Briefs

Supply only the pull request number and commit, the checkout path, for implementation review the assigned facet, and for a later round the adjudication it answers and, where the reviewer's session cannot be resumed, its previous report. PASS means no finding within the reviewer's stated coverage; it is a coverage statement, not a verdict. Reports go on the pull request as comments, one per reviewer and one adjudication. Nothing from a review enters the tree.
...
> Continue the review of the named pull request, now at the named commit, under the brief its reviewer was first given. The previous round is yours, or, if you take over from a reviewer whose session could not be resumed, its report follows and is yours to answer. The adjudication of that round follows; it accepted, rejected or deferred each finding with its reason. Read the whole change again, not only your earlier findings. Judge whether each accepted cure is faithful to its finding and local to its cause, whether anything regressed or a new drift entered, and whether each rejection or deferral holds. Your earlier findings are assertions like any other; renew a rejected one only with evidence the adjudication did not weigh. Work read-only. Return PASS within stated coverage or FINDINGS in the same form as before.

...
| Native geometry/CRS, latitude/longitude, `COMUNE`, `PROVINCIA`, `LOCALITA`, `ALTITUDINE` | Published location. SIT supplies EPSG:32633; workbooks give longitude/latitude columns that establish axes, not a datum. Neither states positional error, so metric use waits for the population row's qualification. The administrative names are established as the observation's own, and 93 of the 235 municipalities are printed in more than one casing, which is exposed as a disagreement and not resolved, exactly as the species spelling is. `LOCALITA` and `ALTITUDINE` are established and compared but carry no value in any record of today's capture. |
| `DOCUMENTO_CONFERMA`, `LNK_DOCUMENTO_SELGE`, `DOCUMENTO_DECRETO` | Literal routes for the report or act owning the next fact, exposed without interpreting the unread document. |
| `SQUADRA`, `TECNICO`, `COD_TECNICI`, the inspector-name columns, `NOME_DISPOSITIVO`, `CODICE_CAMPIONAMENTO`, `STATO`, `NOTE_RILEVATORE`, `CRITICITA_NOTE` | Who performed the observation, with what instrument, in which campaign, and what they noted. One fact under several publisher names: the 2016 infrastructure survey prints a team code and per-inspector columns where later releases print `TECNICO`. `CODICE_CAMPIONAMENTO` names a campaign shared by hundreds of records and is never an identity. A note, a device name and a publication status describe the publication rather than the observation, so they are established and not compared. |
| Protocol, laboratory, cadastral, demarcated-area, protected-plant and removal-decree fields | Carried as the literal the monitoring publisher printed, for the row that owns each fact, and interpreted by nobody here. They lead that row to a source fact; a transcription beside an observation does not fill a report, parcel, permission or removal event. |
...
- Reading the observation's administrative location raised that figure to 16,613:
  `COMUNE` disagrees for 8,203 observations, of which 8,159 already disagreed on
  another field and 44 were not visible as disagreements before. Nearly all of it is
  the same kind of convention as the species one — 93 of the 235 municipalities are
  printed in more than one casing, `Ostuni` beside `OSTUNI` — exposed and not
  resolved, and it is not identity doubt. Only a result disagreement withholds an
  observation from the positive and negative counts, and that figure is unchanged.
- Every absence this row hands a consumer names its cause, and no published field is
  passed over in silence. Over 4,318,100 readings the reader states 83 distinct
  field-and-cause pairs across 61 fields, in ten sentences, distinguishing a field the record does not publish, one published
  carrying no value, one carrying only a sentinel, one carrying a value this reader
  does not interpret, and one published that no row of this stage claims — 8,848
  readings carry that last, 18,814 field entries over 17 names. The reference absence is the
  consequential one: of 223,458, some 12,940 observations are identified by a
  publisher identifier other than the sample key, which the reading now carries under
  its own name. `DistinctObservation.uncorrelated_because` continues to answer why a
  group could not correlate; these answer, for one publication and one field, why this
  reader carries nothing, and they are read on the `DistinctObservation` and its members.
  `detection_days`, `occasion_sets` and `located_positives` return days, identity sets and
  coordinate observations and carry no cause; this row does not change what they return.

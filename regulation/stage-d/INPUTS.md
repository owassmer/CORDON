...
| What the laboratory actually reported | Official reports linked from monitoring and their consequential annex/correction references | One report rendition with source-located sample results and document relationships | Page readings are retained for all 1,180 PDFs (3,092 pages), with 1,178 completed record assemblies. The remaining Codex OAuth batch reused validated Claude blocks; original request provenance survives in retained responses. The ordinary consumption version is `b5abb651434728b7d03c`. All 1,180 document-relationship inventories are marked complete. Population source review and full bidirectional reconciliation remain pending; page coverage and structural completion do not certify source fidelity. Assembly must preserve source-declared field fragments: SELGE 53/2017 splits the host name across physical pages 2–3, and SELGE 116/2016 splits municipality names across pages 3–4 and 5–6. The current materializer drops a fragment-only table and the assembler treats split field values as conflicts. These are local implementation limits, not missing source evidence. Current consumer results and consequential source work below. |

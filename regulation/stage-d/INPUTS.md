...
| What the laboratory actually reported | The official report linked from the monitoring record and consequential report/annex/correction references reached from it | One laboratory report with source-located sample/result occurrences, scoped statements and distinct renditions | The acquired population contains 1,176 distinct PDFs from 1,903 successful route captures. All 15 remaining failures were retried through confirmed Frankfurt egress: CNR36/2024 still returns HTTP404 and fourteen webadf routes HTTP500. The ordinary reader and conservative observation join are implemented; four selected PDFs have all 14 pages accounted for (222 physical rows), while 1,172 PDFs have no assembled reading in the current version. Bounded direct-source checks verify repaired date roles, method/analyte attachment, section qualifications and literal pool-ID components, and a real CNR73 observation reaches unchanged Stage C with unresolved legal qualifications. This does not establish population semantics. The long scanned report still has three directly verified test-date errors and 62 missing publisher-ID projections; six other comparison disagreements were reference errors corrected by direct page inspection. Coordinates and full prose have not received exhaustive independent verification. Repeated representations leave three observations ambiguous; CNR23 client codes still lack a source-supported correspondence to the linked monitoring identifier. Native-text discovery found correction terms in 21 PDFs, but their consequential scopes and event-time relationships remain unfinished, and that search cannot establish completeness on scans. Remaining readings, omitted scanned regions, identity/correction relationships and qualifications remain source work. Deskew is a bounded external probe, not an implemented production repair. Single-page overflow remains a named reading limit; geometric subdivision is not implemented. A literal diagnosis is not official confirmation; the monitoring protocol/laboratory labels remain its own transcription. |

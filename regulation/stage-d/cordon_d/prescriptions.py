...
def lawfully_due(snapshot: Snapshot, at: date, *, instrument: str, governing_references: Iterable[str],
    ...
    `governing_references` is the `case-prescription` record's field: every A row
    of the order's instrument, plus any correction or supplement that names the
    order. A row of the instrument the record does not reach, or a reached row
    without a resolved result, leaves lawful dueness unknown. D adds no condition.
    """
    day = at.isoformat()
    required = {sid for sid, rows in snapshot.stable.items() for row in rows
                if row["instrument_id"] == instrument and row["effective_from"] <= day

...
def cell_components(block, native):
    ...
    for fact in block['facts']:
        ...
        # Cell, clause and literal are compared as the text enters the reading (`decoded`),
        # so the span indexes the retained parent cell whichever layer the reader quoted.
        text = decoded(native[cell['native_cell']]['text'] if 'native_cell' in cell else cell.get('text'))
        quoted, literal = decoded(fact['text']), decoded(fact.get('value'))
        clause = span(text, quoted)
        value = span(quoted, literal)
        ...
        previous.append({'fact': fact, 'span': bounds, 'value': literal})
    return components
            ...
            for raw in table['rows']:
                ...
                for index, (column, cell) in enumerate(zip(table['columns'], raw['cells'])):
                    ...
                    for component in components.get(source_cell, ()):
                        ...
                        field = dict(text=component['value'], role=fact['role'], heading=column['heading'],
                        ...
                        if fact['role'] == 'identifier':
                            field['identifier'] = component['value']
                        cells.append(field)

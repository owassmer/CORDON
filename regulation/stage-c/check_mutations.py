...
MUTATIONS = (
    ...
    ("cordon_c.bindings", '        if truth is True:', '        if truth is not False:',
     "OwnerRulings20260924.test_notice_runs_from_the_earliest_branch_a_finds_true"),
    ("cordon_c.bindings", '    return min(held, key=', '    return max(held, key=',
     "OwnerRulings20260924.test_notice_runs_from_the_earliest_branch_a_finds_true"),
    ("cordon_c.bindings", 'anchor=publication - timedelta(days=1)', 'anchor=publication',
     "OwnerRulings20260924.test_posting_completes_on_the_stated_display_days_counted_inclusively"),
    ("cordon_c.bindings", 'return end_of_day(value, zone) if end else', 'return datetime.combine(value, time(), zone) if end else',
     "OwnerRulings20260924.test_posting_completes_on_the_stated_display_days_counted_inclusively"),
    ...
    ("cordon_c.core", 'return Evaluation(day < source["effective_to_exclusive"], provisions=cited)',

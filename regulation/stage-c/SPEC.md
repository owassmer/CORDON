...
| Spatial predicates | `spatial`: valid 2D geometry, explicit compatible metric CRS, and spatial error bounds. Adopted-area membership includes its boundary; ambiguous precision affects only that classification. Radius/band membership uses distance directly. A geometry with an error bound stands for any true shape between itself shrunk and itself grown by that bound. `partial_parcel` is true when the parcel lies inside the area by more than the combined error, or when the parcel and the area, each shrunk by its own error, overlap with positive area; it is false beyond the combined error and unknown otherwise. `minimum_enclosure` tests containment and clearance from every boundary, including holes; it does not establish legal geography. Their drawn buffers use a circumscribed radius, so polygonal approximation errs toward unknown. |

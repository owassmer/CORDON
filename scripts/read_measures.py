...
from cordon_d.measures import read_measure, retained_measure
...
def main():
    ...
    if args.request:
        if args.sources or args.execute or args.review_instruction:
            parser.error('--request cannot be combined with source-reading options')
        with redirect_stdout(sys.stderr):
            result = retained_measure(args.request, store_root(ROOT))
    else:
        if not args.sources:
            parser.error('Supply source hashes or --request')
        with redirect_stdout(sys.stderr):
